...
export const addProductCategory = (req, res) => {
  (async () => {
    const { label, serviceID, dollar, euro, dinnar } = req.body;
    try {
      const service = await serviceQueries.findById(serviceID, "code");
      if (service === null) {
        return res.status(401).json(serviceNotExist(serviceID));
      }

      const checkExistCategory = service.dataValues.ProductCategories.filter(
      ...
      const { productCategory } = await productCategoryQueries.create({
        label,
      ...
      const product = await productCategoryQueries.findById(
...
export const updateProductCategory = (req, res) => {
  (async () => {
    ...
    try {
      await productCategoryQueries.update({
        field: "id",
        value: id,
        body: { label },
      });
