...
const dirToSave = path.resolve(
  CURRENT_WORKING_DIR,
  "resources/static/assets/excel/codes"
);
    ...
    try {
      ...
      resolve(
        destination
          .split("/")
          .slice(
            destination.split("/").findIndex((ele) => ele === "resources") + 1
          )
          .join("/")
      );
    } catch (error) {
...
const uploadExcel = (isStorage) =>
  multer({ storage: storage(isStorage), fileFilter: excelFilter });
